...
class Select extends BaseComponent {
  ...
  constructor(rootElement) {
    ...
    this.state = this.getProxyState({
      ...this.initialState,
      curreentOptionIndex: this.originalControlElement.selectedIndex,
  ...
  updateUI() {
    const { isExpanded, curreentOptionIndex, selectedOptionElement } =
      this.state;
    ...
    const updateButton = () => {
      ...
      this.buttonElement.setAttribute(
        ...
        this.optionElements[curreentOptionIndex].id
      );
    };
    ...
    const updateOptions = () => {
      this.optionElements.forEach((optionElement, index) => {
        const isCurrent = curreentOptionIndex === index;
  ...
  selectCurrentOption() {
    this.state.selectedOptionElement =
      this.optionElements[this.state.curreentOptionIndex];
  }
  ...
  onClick = (event) => {
    ...
    if (isOptionClick) {
      ...
      this.state.curreentOptionIndex = [...this.optionElements].findIndex(
  ...
  onArrowUpKeyDown = () => {
    ...
    if (this.state.curreentOptionIndex > 0) {
      this.state.curreentOptionIndex--;
    }
  };
  ...
  onArrowDownKeyDown = () => {
    ...
    if (this.state.curreentOptionIndex < this.optionElements.length - 1) {
      this.state.curreentOptionIndex++;
    }
  };
...
export default SelecCollection;
